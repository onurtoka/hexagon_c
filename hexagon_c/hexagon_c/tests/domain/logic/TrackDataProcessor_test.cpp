...
// Bu dosyada, TrackDataProcessor'ın gerçek sistemdeki veri akışını ve gecikme hesaplamalarını test ediyoruz.
// Testin amacı: B_hexagon'dan gelen verinin C_hexagon'da işlenip, doğru şekilde sonuca ulaşmasını ve gecikme değerlerinin hesaplanmasını doğrulamak.
...
protected:
    void SetUp() override {
        ...
        // Bu değerler, sistemdeki veri akışının ve gecikmelerin test edilmesini sağlar.
    }
...
TEST_F(TrackDataProcessorTest, SimulateRealSystemTest) {
    ...
    // 1. Test başlangıcı: B_hexagon'dan gelen verinin simülasyonu ve gecikme değerlerinin yazdırılması
    ...
    // 2. Veriyi işleyiciye gönderiyoruz. Gerçek sistemde olduğu gibi, processor onDataReceived ile veriyi işler.
    ...
    // 4. C_hexagon tarafında veri tekrar işleniyor ve mock sender ile gönderiliyor.
    // Temel doğrulamalar
    ...
    // 6. Testin doğrulama kısmı: Track ID ve gecikme değerlerinin beklenen şekilde olup olmadığı kontrol edilir.
}