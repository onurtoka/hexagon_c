...
/**
 * @brief Input domain value object carrying raw track kinematic & timing attributes.
 * 
 * All temporal values are expressed as microseconds since UNIX epoch.
 */
...
public:
    ...
    long originalUpdateTime{0L};   ///< Original update time (microseconds) - matches ExtrapTrackData.json schema
     ...
     */
    TrackData(int id, double xv, double yv, double zv, double xp, double yp, double zp,
              long ut, long out, long fhst) noexcept
     ...
     */
    TrackData& operator=(const TrackData& other) noexcept = default;
