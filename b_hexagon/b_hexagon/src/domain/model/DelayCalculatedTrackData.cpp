...
bool DelayCalculatedTrackData::isValid() const noexcept {
    ...
    // Check for reasonable time values (positive or zero)
    if (updateTime < 0L || originalUpdateTime < 0L || firstHopSentTime < 0L || 
        firstHopDelayTime < 0L || secondHopSentTime < 0L) {
        return false;
    }
    ...
    // Check that delay calculations make sense (delay should be positive if calculated)
    if (firstHopDelayTime > 0L && secondHopSentTime > 0L) {
        if (secondHopSentTime <= firstHopSentTime) {
            return false; // Second hop should be after first hop
        }
    }
